	...
	<div id="mainContent">
		<form id="wf_formForm">
			<div style="" class="searchZone">
				...
				<div class="searchWrap">
					<span class="searchList">输入框列数</span>
					<span class="searchCont">
						<hx:text property="text_size" required="true" maxlength="10" value="85" msgName="输入框列数" size="10" mask="integer"/>
                    </span>
				</div>
				<div class="searchWrap">
					<span class="searchList">输入框行数</span>
					<span class="searchCont">
						<hx:text property="text_rows" required="true" maxlength="10" value="6" msgName="输入框行数" size="10" mask="integer"/>
                    </span>
				</div>
			</div>
		</form>
...
<script type="text/javascript">
	...
	window.onload = function() {
	    if( UE.plugins[thePlugins].editdom ){//修改时加载
	        ...
	        $G('text_size').value = oNode.getAttribute('cols');
	        $G('text_rows').value = oNode.getAttribute('rows');
	        $('#param_namechn').focus();
	    }else{
	    	$('#param_name').focus();
	    }
	...
	dialog.onok = function (){
	    ...
	    if( !oNode ) {//新建
	        try {
	        	...
	        	var html = '<textarea ';
	            html += ' id = "' + $G('param_name').value + '"';
	            html += ' name = "' + $G('param_name').value + '"';
                html += ' class = "MyAreaBig"';
	            html += ' param_namechn = "' + $G('param_namechn').value + '"';
	            html += ' datadefault = "' + $G('datadefault').value + '"';
	            html += ' cols = "' + $G('text_size').value + '"';
	            html += ' rows = "' + $G('text_rows').value + '"';
	            html += ' myplugins = "'+thePlugins+'"';
	            html += '>';
	            html += '{多行文本：'+($G('datadefault').value==""?$G('param_namechn').value:$G('datadefault').value)+'}' + '</textarea>';
            ...
            oNode.value = $G('datadefault').value;
            oNode.setAttribute('datadefault',$G('datadefault').value);
            oNode.setAttribute('cols',$G('text_size').value);
            oNode.setAttribute('rows',$G('text_rows').value);
            oNode.value = '{多行文本：'+($G('datadefault').value==""?$G('param_namechn').value:$G('datadefault').value)+'}';
